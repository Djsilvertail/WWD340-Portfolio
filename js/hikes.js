  ...
  addHikeListener() {
    ...
    childrenArray.forEach(child => {
      child.addEventListener('click', e => {
        console.log('yay', e, e.currentTarget)
...
function renderHikeList(parent, hikes) {
  hikes.forEach((hike, idx) => {
    console.log(hike, "what are we looking at")
    parent.appendChild(renderOneHikeLight(hike, idx));
    // const hikePicture = document.getElementById(`image-${idx}`);
    // hikePicture.addEventListener("click", (event) => {
    //   console.log('YAY it works!!!!!', event, event.target);
    // });
  });
  // const hikePictures = document.getElementsByClassName('image');
  // console.log(hikePictures, 'adjkfdjsladlajdjfakjfkdjfa');
  // hikePictures.addEventListener("click", () => {
  //   console.log('Picture was clicked!');
  // });
}

function renderOneHikeLight(hike, idx) {
...
<div id="image-${idx}" class="image"><img src="${imgBasePath}${hike.imgSrc}" alt="${hike.imgAlt}"></div>
<div>
        <div>
            <h3>Distance</h3>
            <p>${hike.distance}</p>

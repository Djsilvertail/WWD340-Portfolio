...
function renderShipDetails(shipData) {
  console.log(shipData, 'Ship Details are working');
  let banana = document.querySelector('#banana');
  let textContent = `
      <div>Name: ${shipData.name}</div>
      <div>Model: ${shipData.model}</div>
      <div>Class: ${shipData.starship_class}</div>
      <div>Movies:</div>
      `;
  shipData.films.forEach(film => {
    textContent += `<a href="${film}">${film}</a><br>`;
  });
  banana.innerHTML = textContent;
}
...
function showShips(url = "https://swapi.dev/api/starships/") {
  getShips(url).then(function (data) {
    ...
    if (data.next) {
      console.log(data.next, 'next page should be...')
      ...
      next.onclick = () => {
        console.log('my event is firing')
...
function getShipDetails(url) {
  ...
  getShips(url).then(function (data) {
    console.log(data, 'fuck ya it works');
